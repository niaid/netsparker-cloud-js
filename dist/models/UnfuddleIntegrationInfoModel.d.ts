/**
 * Invicti Enterprise API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: v1
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
import type { IntegrationWizardResultModel } from './IntegrationWizardResultModel';
import type { IntegrationCustomFieldVm } from './IntegrationCustomFieldVm';
/**
 * The Unfuddle integration info
 * @export
 * @interface UnfuddleIntegrationInfoModel
 */
export interface UnfuddleIntegrationInfoModel {
    /**
     * Gets or sets the assignee id.
     * @type {number}
     * @memberof UnfuddleIntegrationInfoModel
     */
    assigneeId?: number;
    /**
     * Gets or sets the due days.
     * @type {number}
     * @memberof UnfuddleIntegrationInfoModel
     */
    dueDays?: number;
    /**
     * Gets or sets the milestone id.
     * @type {number}
     * @memberof UnfuddleIntegrationInfoModel
     */
    milestoneId?: number;
    /**
     * Gets or sets the password.
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    password: string;
    /**
     * Gets or sets the priority.
     * @type {number}
     * @memberof UnfuddleIntegrationInfoModel
     */
    priority: number;
    /**
     * Gets or sets the project id.
     * @type {number}
     * @memberof UnfuddleIntegrationInfoModel
     */
    projectId: number;
    /**
     * Gets or sets the subdomain.
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    subdomain: string;
    /**
     * Gets or sets the username.
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    username: string;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    readonly type?: UnfuddleIntegrationInfoModelTypeEnum;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    readonly genericErrorMessage?: string;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    readonly identifier?: string;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    readonly testMessageBody?: string;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    readonly testMessageTitle?: string;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    readonly webhookUrl?: string;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    name?: string;
    /**
     *
     * @type {number}
     * @memberof UnfuddleIntegrationInfoModel
     */
    integrationVersion?: number;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    accountID?: string;
    /**
     *
     * @type {Array<IntegrationCustomFieldVm>}
     * @memberof UnfuddleIntegrationInfoModel
     */
    customFields?: Array<IntegrationCustomFieldVm>;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    templateType?: UnfuddleIntegrationInfoModelTemplateTypeEnum;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    reopenStatus?: string;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    resolvedStatus?: string;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    titleFormat: string;
    /**
     *
     * @type {IntegrationWizardResultModel}
     * @memberof UnfuddleIntegrationInfoModel
     */
    integrationWizardResultModel?: IntegrationWizardResultModel;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    id?: string;
    /**
     *
     * @type {string}
     * @memberof UnfuddleIntegrationInfoModel
     */
    state?: UnfuddleIntegrationInfoModelStateEnum;
}
/**
 * @export
 */
export declare const UnfuddleIntegrationInfoModelTypeEnum: {
    readonly Jira: "Jira";
    readonly GitHub: "GitHub";
    readonly Tfs: "TFS";
    readonly FogBugz: "FogBugz";
    readonly ServiceNow: "ServiceNow";
    readonly Slack: "Slack";
    readonly GitLab: "GitLab";
    readonly Bitbucket: "Bitbucket";
    readonly Unfuddle: "Unfuddle";
    readonly Zapier: "Zapier";
    readonly AzureDevOps: "AzureDevOps";
    readonly Redmine: "Redmine";
    readonly Bugzilla: "Bugzilla";
    readonly Kafka: "Kafka";
    readonly PagerDuty: "PagerDuty";
    readonly MicrosoftTeams: "MicrosoftTeams";
    readonly Clubhouse: "Clubhouse";
    readonly Trello: "Trello";
    readonly Asana: "Asana";
    readonly Webhook: "Webhook";
    readonly Kenna: "Kenna";
    readonly Freshservice: "Freshservice";
    readonly YouTrack: "YouTrack";
    readonly NetsparkerEnterprise: "NetsparkerEnterprise";
    readonly Splunk: "Splunk";
    readonly Mattermost: "Mattermost";
    readonly Hashicorp: "Hashicorp";
    readonly PivotalTracker: "PivotalTracker";
    readonly CyberArk: "CyberArk";
    readonly DefectDojo: "DefectDojo";
    readonly JazzTeam: "JazzTeam";
    readonly AzureKeyVault: "AzureKeyVault";
    readonly ServiceNowVrm: "ServiceNowVRM";
};
export type UnfuddleIntegrationInfoModelTypeEnum = typeof UnfuddleIntegrationInfoModelTypeEnum[keyof typeof UnfuddleIntegrationInfoModelTypeEnum];
/**
 * @export
 */
export declare const UnfuddleIntegrationInfoModelTemplateTypeEnum: {
    readonly Standard: "Standard";
    readonly Detailed: "Detailed";
};
export type UnfuddleIntegrationInfoModelTemplateTypeEnum = typeof UnfuddleIntegrationInfoModelTemplateTypeEnum[keyof typeof UnfuddleIntegrationInfoModelTemplateTypeEnum];
/**
 * @export
 */
export declare const UnfuddleIntegrationInfoModelStateEnum: {
    readonly Active: "Active";
    readonly Suspended: "Suspended";
};
export type UnfuddleIntegrationInfoModelStateEnum = typeof UnfuddleIntegrationInfoModelStateEnum[keyof typeof UnfuddleIntegrationInfoModelStateEnum];
/**
 * Check if a given object implements the UnfuddleIntegrationInfoModel interface.
 */
export declare function instanceOfUnfuddleIntegrationInfoModel(value: object): boolean;
export declare function UnfuddleIntegrationInfoModelFromJSON(json: any): UnfuddleIntegrationInfoModel;
export declare function UnfuddleIntegrationInfoModelFromJSONTyped(json: any, ignoreDiscriminator: boolean): UnfuddleIntegrationInfoModel;
export declare function UnfuddleIntegrationInfoModelToJSON(value?: Omit<UnfuddleIntegrationInfoModel, 'Type' | 'GenericErrorMessage' | 'Identifier' | 'TestMessageBody' | 'TestMessageTitle' | 'WebhookUrl'> | null): any;
